...
describe('<Menu />', () => {
  it('should render button link', () => {
    ...
    const butoonLink = screen.getByRole('link', { name: 'Open or close menu' });
    const openMenuIcon = screen.getByLabelText('Open menu');

    expect(butoonLink).toBeInTheDocument();
  ...
  it('should open/close menu on button click', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Samuel Carvalho' }));
    expect(screen.getByRole('img', { name: 'Samuel Carvalho' }));
